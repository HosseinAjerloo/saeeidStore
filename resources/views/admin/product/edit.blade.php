...
@section('title')
    <title>پنل | ویرایش محصول جدید</title>
@endsection
@section('content')
    <main class="flex-1 p-4 sm:p-6 lg:p-8">
        <div class="mx-auto max-w-7xl">
            <section
                ...
                <div class="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        ...
                        <h2 class="text-2xl font-extrabold text-white sm:text-3xl">ویرایش محصول</h2>
            ...
            <form id="createProductForm" action="{{route('admin.product.update',$product)}}" method="post" enctype="multipart/form-data" novalidate="">
                @csrf
                @method('PUT')
                    ...
                    <div class="space-y-5 xl:col-span-8">
                        <section class="form-section glass-card animate-fade-up stagger-1 overflow-hidden p-0">
                            ...
                            <div class="grid grid-cols-1 gap-5 p-5 sm:grid-cols-2 sm:p-7">
                                <label class="field-group"><span class="field-label">
                                        نام محصول
                                        <span class="text-rose">*</span>
                                    </span><span class="field-shell">
                                        <svg viewBox="0 0 24 24"><path d="M5 5h14v14H5zM9 9h6v6H9z">

                                            </path>
                                            </svg>
                                        <input id="productName" name="name" type="text" required="" maxlength="255" value="{{$product->name??''}}"
                                            placeholder="مثلاً هدفون بی‌سیم سونی"></span>
                                    <span id="nameError" class="mt-2 hidden text-[11px] text-rose">نام محصول را وارد کنید.</span>
                                </label>
                                ...
                                <label class="field-group">
                                    ...
                                    <select id="brandInput" name="brand_id" class="native-select">
                                       @foreach($brands as $brand)
                                            <option @selected(old('brand_id',$product->brand_id)==$brand->id) value="{{$brand->id}}">{{$brand->name}}</option>
                                        @endforeach
                                    </select>
                        ...
                        <section class="form-section glass-card animate-fade-up stagger-2 overflow-hidden p-0">
                            <div class="section-heading">
                                ...
                                <span class="mr-auto chip bg-aqua-500/10 text-aqua-300">معرفی محصول</span>
                            </div>
                            <div class="space-y-5 p-5 sm:p-7">
                                <label class="field-group">
                                    ...
                                    <span
                                        ...
                                        <svg class="mt-3" viewBox="0 0 24 24"><path
                                                d="M4 6h16M4 11h12M4 16h8"></path>
                                        </svg>
                                        <textarea id="shortDescription" name="short_description" rows="3" maxlength="250" placeholder="خلاصه‌ای جذاب از مهم‌ترین ویژگی‌های محصول...">{{old('short_description',$product->short_description)}}</textarea>
                                    </span>
                                    <span class="mt-2 flex justify-between text-[10px] text-slate-600">
                                        <span>در کارت و ابتدای صفحه محصول نمایش داده می‌شود.</span>
                                        <span id="shortCount">۰ / ۲۵۰</span>
                                    </span>
                                </label>
                                <label class="field-group">
                                    <span class="field-label">توضیحات کامل</span>
                                    <span class="field-shell items-start"><svg class="mt-3" viewBox="0 0 24 24">
                                            <path d="M4 5h16M4 10h16M4 15h10M4 19h7"></path>
                                        </svg>
                                        <textarea id="description" name="description" rows="7" maxlength="5000" placeholder="مشخصات، کاربردها و جزئیات کامل محصول را وارد کنید...">{{old('description',$product->description)}}</textarea>
                                    </span>
                                ...
                                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                    <div class="field-group"><span class="field-label">وضعیت محصول</span><label
                                            class="account-status"><span
                                                ...
                                                class="relative"><input @if(old('is_active',$product->is_active)=='1') checked="checked" @endif id="activeInput" name="is_active" type="checkbox"
                                                                        value="1"  class="peer sr-only"><span
                                    ...
                                    <div class="field-group"><span class="field-label">محصول ویژه</span><label
                                            class="account-status"><span
                                                ...
                                                class="relative">
                                                <input id="featuredInput"  @if(old('is_featured',$product->is_featured)=='1') checked="checked" @endif name="is_featured"
                                                                        type="checkbox" value="1" class="peer sr-only">
                                                <span
                    ...
                    <aside class="xl:col-span-4">
                        <section class="profile-preview glass-card overflow-hidden p-0 xl:sticky xl:top-24">
                            <div id="imagePreview"
                                ...
                                <img id="previewImg" class="absolute inset-0 @if(!$product->image) hidden @endif h-full w-full object-cover"
                            ...
                            <div class="p-6">
                                <div class="flex flex-wrap gap-2"><span id="groupPreview"
                                                                        class="chip bg-brand-500/10 text-brand-300"> گروه:{{$product->group->name??''}}</span><span
                                        id="brandPreview" class="chip bg-aqua-500/10 text-aqua-300"> برند:{{$product->brand->name??''}}</span></div>
                                <h3 id="namePreview" class="mt-4 text-lg font-extrabold text-white">نام محصول {{$product->name}}</h3>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#category-tree').on('loaded.jstree', function () {
                ...
                if ({{old('group_id',$product->group_id)}}) {
                    $('#category-tree')
                        .jstree(true)
                        .select_node({{old('group_id',$product->group_id)}});
                }
