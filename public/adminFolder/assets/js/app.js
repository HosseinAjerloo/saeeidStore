...
(function () {
  ...
  /* ---------- تم روشن / تیره مشترک بین تمام صفحه‌ها ---------- */
  const themeRoot = document.documentElement;
  const themeStorageKey = "huma-admin-theme";
  ...
  function applyTheme(theme) {
    ...
    themeRoot.classList.toggle("dark", !isLight);
    themeRoot.classList.toggle("theme-light", isLight);
    themeRoot.style.colorScheme = isLight ? "light" : "dark";
  ...
  if (headerBar && !document.getElementById("themeToggle")) {
    const themeButton = document.createElement("button");
    themeButton.id = "themeToggle";
    themeButton.type = "button";
    themeButton.className = "icon-btn theme-toggle shrink-0";
    headerBar.appendChild(themeButton);
    applyTheme(readTheme());
    themeButton.addEventListener("click", () => {
      const nextTheme = themeRoot.classList.contains("theme-light") ? "dark" : "light";
      try {
        localStorage.setItem(themeStorageKey, nextTheme);
      } catch (_) {
        /* ذخیره‌سازی ممکن است در بعضی مرورگرها غیرفعال باشد. */
      }
